...
const petIdsSchema = z
  .array(z.string().min(1).max(100))
  .min(1)
  .max(100)
  .refine(items => new Set(items).size === items.length, "宠物关联不能重复");
const optionalPetIdsSchema = z
  .array(z.string().min(1).max(100))
  .max(100)
  .refine(items => new Set(items).size === items.length, "宠物关联不能重复");
...
export const legacyPetBackupSchema = z.object({
  ...
  profile: z.object({ ...profileFields, name: z.string().max(100) }),
  records: z.array(z.object(recordFields)).max(10_000),
  photos: z.array(z.object(photoFields)).max(5_000),
  supplies: z.array(z.object(supplyFields)).max(5_000),
  homeCardTypes: homeCardsSchema,
});

export const petBackupSchema = z
  .object({
    format: z.literal("pet-observation-backup"),
    version: z.literal(2),
    exportedAt: z.string().datetime(),
    ...
    records: z
      .array(
        z.object({
          ...
          petIds: petIdsSchema.optional(),
          ...recordFields,
        })
      )
      .max(50_000),
    photos: z
      .array(z.object({ petId: z.string().min(1).max(100), ...photoFields }))
      .max(10_000),
    supplies: z
      .array(
        z.object({
          petId: z.string().min(1).max(100).optional(),
          petIds: optionalPetIdsSchema.optional(),
          ...supplyFields,
        })
      )
      .max(10_000),
  ...
  .superRefine((backup, context) => {
    ...
    for (const pet of backup.pets) {
      if (petIds.has(pet.id))
        context.addIssue({ code: "custom", message: "宠物 ID 重复" });
      petIds.add(pet.id);
    }
    ...
    for (const item of [
      ...
      if (ids.has(item.id))
        context.addIssue({ code: "custom", message: "数据 ID 重复" });
      ids.add(item.id);
      if ("petId" in item && item.petId && !petIds.has(item.petId)) {
        context.addIssue({ code: "custom", message: "备份包含无效的宠物关联" });
      }
      if ("petIds" in item && item.petIds?.some(id => !petIds.has(id))) {
        context.addIssue({
          code: "custom",
          message: "备份包含无效的多宠物关联",
        });
      }
    }
    ...
    pets: [
      {
        ...
        species: "dog",
        ...legacy.data.profile,
        name: legacy.data.profile.name || "未命名宠物",
      },
    ],
    records: legacy.data.records.map(record => ({
      ...record,
      petId,
      petIds: [petId],
    })),
...
export function parsePetBackupText(value: string): PetBackup {
  if (value.length > 256 * 1024 * 1024)
    throw new Error("备份文件不能超过 256 MiB");
  let parsed: unknown;
  try {
