...
export function useDogData() {
  ...
  const afterDataChanged = (refresh: () => Promise<unknown>) => () => {
    void refresh();
  ...
  return {
    isLoading: recordsQ.isLoading || profileQ.isLoading || photosQ.isLoading || suppliesQ.isLoading,
    ...
    setDailyPhoto: (date: string, photo: string, caption: string) => setPhotoM.mutate({ date, photo, caption }),
